...
notify_cmd_clip_base="notify-send -t 5000 -h string:x-canonical-private-synchronous:shot-notify"
notify_cmd_clip="${notify_cmd_clip_base} -i ${iDIR}/picture.png"
...
shotclipboard() {
    ...
    if [[ -z "$area" ]]; then
        "${sDIR}/Sounds.sh" --error
        ${notify_cmd_NOT} "Screenshot" "Capture Canceled"
        return 1
    fi

    if grim -g "$area" - | wl-copy; then
        "${sDIR}/Sounds.sh" --screenshot
        ${notify_cmd_clip} "Screenshot" "Copied to Clipboard"
    else
        "${sDIR}/Sounds.sh" --error
        ${notify_cmd_NOT} "Screenshot" "Failed to Copy"
    fi
}
...
if [[ "$1" == "--now" ]]; then
	...
	echo -e "Available Options : --now --in5 --in10 --win --area --clipboard --active --swappy"
fi
